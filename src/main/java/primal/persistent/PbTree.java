...
public class PbTree<T> implements PerTree<T> {
	...
	public boolean validate() {
		return Puller.of(root).isAll(this::validate) ? true : fail();
	}
	...
	public Streamlet<T> streamlet() {
		return new Streamlet<>(() -> stream(root, null, null));
	}

	private Puller<T> stream(List<Slot> node, T start, T end) {
		var i0 = start != null ? new FindSlot(node, start).i : 0;
		var i1 = end != null ? new FindSlot(node, end, true).i + 1 : node.size();

		if (i0 < i1)
			return Puller.of(node.subList(i0, i1)).concatMap(slot -> {
				if (slot.slots != null)
					return stream(slot.slots, start, end);
				else
					return slot.pivot != null ? Puller.of(slot.pivot) : Puller.of();
			});
		else
			return Puller.of();
	}
