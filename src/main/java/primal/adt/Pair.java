...
public class Pair<K, V> {
	...
	public static <K extends Comparable<? super K>, V extends Comparable<? super V>> Comparator<Pair<K, V>> comparator() {
		return (pair0, pair1) -> {
			...
			c = c == 0 ? Compare.objects(pair0.k, pair1.k) : c;
	...
	public static <K extends Comparable<? super K>, V> Comparator<Pair<K, V>> comparatorByFirst() {
		return Comparator.comparing(pair -> fst(pair), Compare::objects);
	}
	...
	public boolean equals(Object object) {
		if (Get.clazz(object) == Pair.class) {
			var other = (Pair<?, ?>) object;
			return Equals.ab(k, other.k) && Equals.ab(v, other.v);
		} else
			return false;
	}
