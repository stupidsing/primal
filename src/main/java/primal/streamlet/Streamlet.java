...
public class Streamlet<T> implements StreamletDefaults<T, Opt<T>, Predicate<T>, Puller<T>, Sink<T>, Source<T>> {
	...
	public <K, V> Streamlet2<K, List<T>> groupBy(Fun<T, K> keyFun) {
		return streamlet2(() -> spawn().groupBy(keyFun));
	}

	public <K, V1> Streamlet2<K, V1> groupBy(Fun<T, K> keyFun, Fun<Streamlet<T>, V1> fun) {
		return streamlet2(() -> spawn().groupBy(keyFun).mapValue(list -> fun.apply(streamlet(() -> Puller.of(list)))));
	}
	...
	public <K, V> Map<K, V> toMap(Fun<T, K> keyFun, Fun<T, V> valueFun) {
		return spawn().map2(keyFun, valueFun).groupBy().mapValue(values -> streamlet(values).uniqueResult()).toMap();
	}
	...
	public <K, V> ListMultimap<K, V> toMultimap(Fun<T, K> keyFun, Fun<T, V> valueFun) {
		var puller = spawn().map2(keyFun, valueFun).groupBy();
		var map0 = new HashMap<K, List<V>>();
		var map1 = puller.isAll((k, v) -> map0.put(k, v) == null || failBool("duplicate key " + k)) ? map0 : null;
		return new ListMultimap<>(map1);
	}
