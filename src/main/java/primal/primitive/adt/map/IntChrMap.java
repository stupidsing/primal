...
public class IntChrMap {
	...
	public boolean equals(Object object) {
		if (object instanceof IntChrMap other) {
	...
	public void forEach(IntChrSink sink) {
		var pair = IntChrPair.of((int) 0, (char) 0);
		var source = source_();
	...
	public IntObjPuller<Character> puller() {
		return IntObjPuller.of(new IntObjSource<Character>() {
	...
	private void store(int key, char v1) {
		var index = index(key);
		if (vs[index] == empty) {
			ks[index] = key;
			vs[index] = v1;
